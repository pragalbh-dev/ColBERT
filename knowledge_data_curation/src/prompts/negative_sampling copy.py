...
NEGATIVE_SUBCHAIN_SAMPLING_PROMPT='''Find industries NOT related to the target.

For subchains (short industry paths), focus on the CORE FUNCTION:

Target: "Gastroenterology" (from Healthcare)
Main function: Digestive system medical care
✓ Hard Negative: "Dermatology" (different medical specialty)
✓ Soft Negative: "Retail" (completely different)
✗ WRONG: "Medical" (parent category)
✗ WRONG: "Healthcare" (same domain)

Target: "Solar Energy" (from Energy & Utilities)  
Main function: Renewable power generation
✓ Hard Negative: "Wind Energy" (different energy source)
✓ Soft Negative: "Gaming" (completely different)
✗ WRONG: "Energy" (parent category)
✗ WRONG: "Power" (same domain)

SIMPLE RULE: If target is "X", never include anything containing "X" or its parent domain.

OUTPUT:
{
  "hard_negatives": ["item1", "item2"],
  "soft_negatives": ["item3", "item4"]  
}
